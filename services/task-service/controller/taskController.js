...
const createTask = async (req, res) => {
  try {
    ...
    return res.status(201).json({ stats: "good", content: task });
  } catch (err) {
    console.error("Error in createTask:", err);
    return res.status(500).json({ stats: "bad", content: err.message });
  }
};
...
const getTask = async (req, res) => {
  try {
    ...
    if (!task) {
      return res.status(404).json({ stats: "bad", content: "Task not found" });
    }

    return res.status(200).json({ stats: "good", content: task });
  } catch (err) {
...
const getTasks = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res.status(400).json({ stats: "bad", content: "Missing userId" });
    }
...
const updateTask = async (req, res) => {
  try {
    ...
    return res.status(200).json({ stats: "good", content: updated });
  } catch (err) {
...
const deleteTask = async (req, res) => {
  try {
    ...
    return res.status(200).json({ stats: "good", content: "Task deleted successfully" });
  } catch (err) {
...
const changeStatus = async (req, res) => {
  try {
    ...
    if (!status) {
      return res.status(400).json({ stats: "bad", content: "Missing status field" });
    }
